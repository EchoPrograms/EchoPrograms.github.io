...
function setTableDefaults() {
	for(var i = 1; i < attractionTable.childNodes[0].childNodes.length; i ++) {
		for(var k = 1; k < attractionTable.childNodes[0].childNodes.length; k ++) {
			console.log(Object.keys(colorMapping)[i - 1])
...
function generateInteractionMatrix() {
	for(var i = 2; i < attractionTable.childNodes[0].childNodes.length; i += 2) {
		for(var k = 3; k < attractionTable.childNodes[0].childNodes.length; k += 2) {
			interactionMatrix[Object.keys(colorMapping)[i/2 - 1]][Object.keys(colorMapping)[(k - 1) / 2 - 1]] = attractionTable.childNodes[0].childNodes[i].childNodes[k].childNodes[0].value 
		}
	}
}
...
class Particle {
	...
	calculatePhysics() {
		for(var i = 0; i < particles.length; i++) {
			...
			if(distance > simDistance || distance < 0 || isNaN( xDiff / distance * f * parseInt(interactionMatrix[this.color][particles[i].color]) * 10 * deltaTime)) continue
		...
		var distance = (xDiff**2 + yDiff**2)**(1/2)
		if(distance / zoomOutLevel < effectRadius.value && distance > 0 && dragging && mouseButton == 1) {
